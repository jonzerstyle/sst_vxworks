...
//	double asave;
	...
	if (future[FTBEAM] <= dsst.date+Time && dsst.remcom != 0 && condit != IHDOCKED) {
...
//	future[FSNOVA] = asave;
	...
	if (alldone || dsst.galaxy[quadx][quady] == 1000 || justin != 0) return 1;
	return 0;
}
...
void preport(void) {
	...
	skip(1);
	for (i = 1; i <= inplan; i++) {
		if (dsst.plnets[i].known
#ifdef DEBUG
			|| ( idebug && dsst.plnets[i].x !=0)
#endif
			) {
			iknow = 1;
#ifdef DEBUG
			if (idebug && dsst.plnets[i].known==0) proutn("(Unknown) ");
#endif
			cramlc(1, dsst.plnets[i].x, dsst.plnets[i].y);
			proutn("   class ");
			proutn(classes[dsst.plnets[i].pclass]);
			proutn("   ");
			if (dsst.plnets[i].crystals == 0) proutn("no ");
			prout("dilithium crystals present.");
			if (dsst.plnets[i].known==2) 
...
void sensor(void) {
	...
	proutn(" is of class ");
	proutn(classes[dsst.plnets[iplnet].pclass]);
	prout(".");
	if (dsst.plnets[iplnet].known==2) 
		prout("         Sensors show Galileo still on surface.");
	proutn("         Readings indicate");
	if (dsst.plnets[iplnet].crystals == 0) proutn(" no");
	prout(" dilithium crystals present.\"");
	if (dsst.plnets[iplnet].known == 0) dsst.plnets[iplnet].known = 1;
	return;
}
...
void beam(void) {
	...
	if (damage[DTRANSP] != 0) {
		...
		if (damage[DSHUTTL]==0 && (dsst.plnets[iplnet].known==2 || iscraft == 1)) {
			skip(1);
	...
	if (landed==1) {
		...
		/* Going to planet */
		if (dsst.plnets[iplnet].crystals==0) {
	...
	landed = -landed;
	if (landed==1 && dsst.plnets[iplnet].known==2) {
...
void mine(void) {
	...
	Time = (0.1+0.2*Rand())*dsst.plnets[iplnet].pclass;
	if (consumeTime()) return;
...
void shuttle(void) {
	...
	if(damage[DSHUTTL] != 0.0) {
		if (damage[DSHUTTL] == -1.0) {
			if (inorbit && dsst.plnets[iplnet].known == 2)
	...
	if ((dsst.plnets[iplnet].known != 2) && iscraft != 1) {
	...
	if (Time >= 0.8*dsst.remtime) {
		...
		cramf(100*Time/dsst.remtime,0,4);
		prout("% of our");
...
void deathray(void) {
	...
	if (r > .30) {
		...
		skip(2);
		while (nenhere > 0) {
			deadkl(kx[1],ky[1],quadsst[kx[1]][ky[1]],kx[1],ky[1]);
		}
	...
	if (r <= 0.75) {
		...
		for (i=1; i<=10; i++)
			for (j=1; j<=10; j++)
				if (quadsst[i][j] == IHDOT) quadsst[i][j] = IHQUEST;
