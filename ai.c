...
static int tryexit(int lookx, int looky, int ienm, int loccom, int irun) {
    ...
    dsst.galaxy[quadx][quady] -= 100;
    dsst.galaxy[iqx][iqy] += 100;
    if (ienm==IHS) {
        ishere=0;
        iscate=0;
        ientesc=0;
        isatb=0;
        future[FSCMOVE]=0.2777+dsst.date;
        future[FSCDBAS]=1e30;
        dsst.isx=iqx;
        dsst.isy=iqy;
    }
        ...
        for (l=1; l<=dsst.remcom; l++) {
            if (dsst.cx[l]==quadx && dsst.cy[l]==quady) {
                dsst.cx[l]=iqx;
                dsst.cy[l]=iqy;
                break;
            }
...
static void movebaddy(int comx, int comy, int loccom, int ienm) {
    ...
    for (ll = 1; ll <= nsteps; ll++) {
        ...
        while (attempts++ < 20 && !success) {
            ...
            else if (quadsst[lookx][looky] != IHDOT) {
                /* See if we should ram ship */
                if (quadsst[lookx][looky] == ship &&
                        (ienm == IHC || ienm == IHS)) {
...
void movcom(void) {
    ...
    if (skill > SGOOD) for (i = 1; i <= nenhere; i++) {
        ix = kx[i];
        iy = ky[i];
        if (quadsst[ix][iy] == IHK || quadsst[ix][iy] == IHR)
            movebaddy(ix, iy, i, quadsst[ix][iy]);
    }
...
static int checkdest(int iqx, int iqy, int flag, int *ipage) {
    ...
    if ((iqx==quadx && iqy==quady) ||
            ...
            dsst.galaxy[iqx][iqy] > 899) return 1;
    if (flag) {
        /* Avoid quadrants with bases if we want to avoid Enterprise */
        for (i = 1; i <= dsst.rembase; i++)
            if (dsst.baseqx[i]==iqx && dsst.baseqy[i]==iqy) return 1;
    }

    /* do the move */
    dsst.galaxy[dsst.isx][dsst.isy] -= 100;
    dsst.isx = iqx;
    dsst.isy = iqy;
    dsst.galaxy[dsst.isx][dsst.isy] += 100;
    if (iscate) {
        ...
        future[FSCDBAS]=1e30;
        for (i = 1; i <= nenhere; i++) 
            if (quadsst[kx[i]][ky[i]] == IHS) break;
        quadsst[kx[i]][ky[i]] = IHDOT;
        kx[i] = kx[nenhere];
        ky[i] = ky[nenhere];
    ...
    for (i = 1; i <= inplan; i++) {
        if (dsst.plnets[i].x==dsst.isx && dsst.plnets[i].y==dsst.isy &&
                dsst.plnets[i].crystals == 1) {
            /* destroy the planet */
            dsst.plnets[i] = nulplanet;
            dsst.newstuf[dsst.isx][dsst.isy] -= 1;
            if (REPORTS) {
...
void scom(int *ipage) {
    ...
    flag = ((dsst.killc+dsst.killk)/(dsst.date+0.01-indate) < 0.1*skill*(skill+1.0) ||
            (dsst.date-indate) < 3.0);
    if (iscate==0 && flag) {
        /* compute move away from Enterprise */
        ideltax = dsst.isx-quadx;
        ideltay = dsst.isy-quady;
        if (sqrt(ideltax*(double)ideltax+ideltay*(double)ideltay) > 2.0) {
            /* circulate in space */
            ideltax = dsst.isy-quady;
            ideltay = quadx-dsst.isx;
        }
    }
    else {
        ...
        for (i2 = 1; i2 <= dsst.rembase; i2++) {
            i = basetbl[i2];	/* bug in original had it not finding nearest*/
            ibqx = dsst.baseqx[i];
            ibqy = dsst.baseqy[i];
            if ((ibqx == quadx && ibqy == quady) ||
                    (ibqx == batx && ibqy == baty) ||
                    dsst.galaxy[ibqx][ibqy] > 899) continue;
            /* if there is a commander, an no other base is appropriate,
               we will take the one with the commander */
            for (j = 1; j <= dsst.remcom; j++) {
                if (ibqx==dsst.cx[j] && ibqy==dsst.cy[j] && ifindit!= 2) {
                    ifindit = 2;
                    iwhichb = i;
                    break;
                }
            }
            if (j > dsst.remcom) { /* no commander -- use this one */
                ifindit = 1;
                iwhichb = i;
                break;
            }
        ...
        ibqx = dsst.baseqx[iwhichb];
        ibqy = dsst.baseqy[iwhichb];
        /* decide how to move toward base */
        ideltax = ibqx - dsst.isx;
        ideltay = ibqy - dsst.isy;
    }
    ...
    iqx = dsst.isx + ideltax;
    iqy = dsst.isy + ideltax;
    ...
    else for (i=1; i<=dsst.rembase; i++) {
        ibqx = dsst.baseqx[i];
        ibqy = dsst.baseqy[i];
        if (ibqx==dsst.isx && ibqy == dsst.isy && dsst.isx != batx && dsst.isy != baty) {
            /* attack the base */
            if (flag) return; /* no, don't attack base! */
            iseenit = 0;
            isatb=1;
            future[FSCDBAS] = dsst.date + 1.0 +2.0*Rand();
            if (batx != 0) future[FSCDBAS] += future[FCDBAS]-dsst.date;
            if (!REPORTS)
                return; /* no warning */
            iseenit = 1;
            if (*ipage == 0)  pausesst(1);
...
void movetho(void) {
    ...
    if (quadsst[idx][idy]!= IHDOT && quadsst[idx][idy]!= IHWEB) return;
    quadsst[ithx][ithy] = IHWEB;
    ...
    if (ithx != idx) {
        ...
        while (ithx != idx) {
            ithx += im;
            if (quadsst[ithx][ithy]==IHDOT) quadsst[ithx][ithy] = IHWEB;
        }
    }
    else if (ithy != idy) {
        /* move in y axis */
    ...
    for (i = 1; i < 11; i++) {
        if (quadsst[1][i]!=IHWEB && quadsst[1][i]!=IHT) return;
        if (quadsst[10][i]!=IHWEB && quadsst[10][i]!=IHT) return;
        if (quadsst[i][1]!=IHWEB && quadsst[i][1]!=IHT) return;
        if (quadsst[i][10]!=IHWEB && quadsst[i][10]!=IHT) return;
    }
