...
/*int getch(void);
*/
extern unsigned int sst_doc_len;
extern unsigned char sst_doc[];
...
/* Compared to original version, I've changed the "help" command to
   ...
   from, I added the emexit commandsst.
...
static void helpme(void) {
    ...
    if (i == 23) {
        ...
        //printf("i: %d\n", i);
        //printf("commands: %s\n", commands[i]);
        while ((cmdbuf[j+13] = toupper(commands[i][j])) != 0) {
            //printf("toupper: %c\n", commands[i][j]);
            j++;
        };
    }
    ...
    do {
        ...
        char c;
        size_t rc;
        //fgets hangs on vxWorks... Ugh
        //  try fread instead and manage grabbing lines
        while ((read(fp, &c, 1) == 1) && (c != 0x0a)) {
            linebuf[idx++] = c;
            //printf("%c %d %2x\n", c, idx, c);
        }
...
#ifndef LINUX
    memDevDelete("/mem/sst_doc");
    //VxWorks hangs up when closing fp - all kinds of wierd crap with vxWorks
#else
    fclose(fp);
#endif
}
    ...
    while (TRUE) { /* command loop */
        ...
        switch (i) { /* command switch */
            ...
            case 2:			// phasers
                ...
                if (ididit) {
#ifdef CLOAKING
                    if (irhere && dsst.date >= ALGERON && !isviolreported) {
        ...
        for (;;) {
            ...
            if (dsst.galaxy[quadx][quady] == 1000) { // Galaxy went Nova!
                atover(0);
                continue;
            }
            ...
            if (hitme && justin==0) {
                attack(2);
                if (alldone) break;
                if (dsst.galaxy[quadx][quady] == 1000) {	// went NOVA! 
                    atover(0);
                    hitme = TRUE;
                    continue;
                }
...
int sst(int argc, char **argv) {
    ...
    void prouts(char *s) {
        ...
        while (*s) {
#ifndef LINUX
            endTime = (unsigned int)(sysClkRateGet() * 0.05) + tickGet();
            while (tickGet() < endTime); 
#else
    ...
    int isit(char *s) {
        ...
        return strncmp(s, citem, maxsst(1, strlen(citem))) == 0;
    ...
    void debugme(void) {
        ...
        if (ja() != 0) {
            ...
            for (i=1; i <= ndevice; i++) {
                ...
                if (key == IHALPHA &&  isit("y")) {
                    damage[i] = 10.0;
                    if (i == DRADIO) stdamtim = dsst.date;
                }
            }
        }
        ...
        if (ja() != 0) {
            ...
            for (i = 1; i < NEVENTS; i++) {
                ...
                cramf(future[i]-dsst.date, 8, 2);
                chew();
                proutn("  ?");
                key = scan();
                if (key == IHREAL) {
                    future[i] = dsst.date + aaitem;
                }
            }
            chew();
        }
