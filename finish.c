    ...
    if (nenhere != 0) {
        ...
        while (l <= nenhere) {
            if (kpower[l]*kdist[l] <= whammo) 
                deadkl(kx[l],ky[l], quadsst[kx[l]][ky[l]], kx[l], ky[l]);
            l++;
        }
    }
    finish(FDILITHIUM);
...
void finish(FINTYPE ifin) {
    ...
    switch (ifin) {
        case FWON: // Game has been won
            ...
            if (alive) {
                ...
                badpt = 5.*dsst.starkl + casual + 10.*dsst.nplankl +
                    45.*nhelp+100.*dsst.basekl;
                if (ship == IHF) badpt += 100.0;
                else if (ship == 0) badpt += 200.0;
                if (badpt < 100.0) badpt = 0.0;	// Close enough!
                if (dsst.date-indate < 5.0 ||
                        // killsPerDate >= RateMax
                        (dsst.killk+dsst.killc+dsst.nsckill)/(dsst.date-indate) >=
    ...
    alive = 0;
    if (dsst.remkl != 0) {
        double goodies = dsst.remres/inresor;
        double baddies = (dsst.remkl + 2.0*dsst.remcom)/(inkling+2.0*incom);
...
void score(int inGame) {
    double timused = dsst.date - indate;
    int ithperd, iwon, klship;
    int dnromrem = dsst.nromrem; // Leave global value alone

    if (!inGame) pausesst(0);

    iskill = skill;
    if ((timused == 0 || dsst.remkl != 0) && timused < 5.0) timused = 5.0;
    perdate = (dsst.killc + dsst.killk + dsst.nsckill)/timused;
    ...
    iscore = 10*dsst.killk + 50*dsst.killc + ithperd + iwon
        - 100*dsst.basekl - 100*klship - 45*nhelp -5*dsst.starkl - casual
        + 20*dsst.nromkl + 200*dsst.nsckill - 10*dsst.nplankl + dnromrem;
#ifdef CLOAKING
    iscore -= 100*ncviol;
#endif
#ifdef CAPTURE
    ...
    if (dsst.nromkl)
        printf(dsst.nromkl> 1 ? "%6d Romulan ships destroyed            %5d\n" : "%6d Romulan ship destroyed             %5d\n",
                dsst.nromkl, 20*dsst.nromkl);
    if (dnromrem)
        printf(dnromrem > 1 ? "%6d Romulan ships captured             %5d\n" : "%6d Romulan ship captured              %5d\n",
                dnromrem, dnromrem);
    if (dsst.killk)
        printf(dsst.killk > 1 ? "%6d ordinary Klingon ships destroyed   %5d\n" : "%6d ordinary Klingon ship destroyed    %5d\n",
                dsst.killk,  10*dsst.killk);
    if (dsst.killc)
        printf(dsst.killc > 1 ? "%6d Klingon Commander ships destroyed  %5d\n" : "%6d Klingon Commander ship destroyed   %5d\n",
                dsst.killc, 50*dsst.killc);
    if (dsst.nsckill)
        printf("%6d Super-Commander ship destroyed     %5d\n",
                dsst.nsckill, 200*dsst.nsckill);
    if (ithperd)
...
#endif
    if (dsst.starkl)
        printf(dsst.starkl > 1 ? "%6d stars destroyed by your action     %5d\n" : "%6d star destroyed by your action      %5d\n",
                dsst.starkl, -5*dsst.starkl);
    if (dsst.nplankl)
        printf(dsst.nplankl > 1 ? "%6d planets destroyed by your action   %5d\n" : "%6d planet destroyed by your action    %5d\n",
                dsst.nplankl, -10*dsst.nplankl);
    if (dsst.basekl)
        printf(dsst.basekl > 1 ? "%6d bases destroyed by your action     %5d\n" : "%6d base destroyed by your action      %5d\n",
                dsst.basekl, -100*dsst.basekl);
    if (nhelp)
