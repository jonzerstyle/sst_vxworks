...
void lmove(void) {
...
#ifdef CLOAKING
    if (iscloaked && dsst.date+Time >= future[FTBEAM])
	...
	if (dsst.date+Time >= future[FTBEAM]) {
		trbeam = 1;
		condit = IHRED;
		dist = dist*(future[FTBEAM]-dsst.date)/Time + 0.1;
		Time = future[FTBEAM] - dsst.date + 1e-5;
	}
		...
		for (l = 1; l <= n; l++) {
			...
			if (ix < 1 || ix > 10 || iy < 1 || iy > 10) {
				...
				if (nenhere != 0 && iattak != 2
					...
					if (dsst.galaxy[quadx][quady] != 1000) attack(0);
					if (alldone) return;
				}
...
label100:
	...
	if (nenhere) {
		...
		sortkl();
		if (dsst.galaxy[quadx][quady] != 1000 && iattak == 0)
			attack(0);
...
void warp(int i) {
	...
	if (i!=2) { /* Not WARPX entry */
		...
		if (Time >= 0.8*dsst.remtime) {
			skip(1);
			prout("First Officer Spock- \"Captain, I compute that such");
			proutn("  a trip would require approximately ");
			cramf(100.0*Time/dsst.remtime, 0, 2);
...
void atover(int igrab) {
	...
	do {
		...
		starch[quadx][quady] = damage[DRADIO] > 0.0 ? dsst.galaxy[quadx][quady]+1000:1;
	...
	} while (dsst.galaxy[quadx][quady] == 1000);
	if (dsst.remkl==0) finish(FWON); /* Snova killed remaining enemy. */
}

void timwrp() {
	int l, ll, gotit;
	prout("***TIME WARP ENTERED.");
	if (dsst.snap && Rand() < 0.5) {
		/* Go back in time */
		proutn("You are traveling backwards in time ");
		cramf(dsst.date-snapsht.date, 0, 2);
		prout(" stardates.");
		dsst = snapsht;
		dsst.snap = 0;
		if (dsst.remcom) {
			future[FTBEAM] = dsst.date + expran(intime/dsst.remcom);
			future[FBATTAK] = dsst.date + expran(0.3*intime);
		}
		future[FSNOVA] = dsst.date + expran(0.5*intime);
		future[FSNAP] = dsst.date +expran(0.25*dsst.remtime); /* next snapshot will
													   be sooner */
		if (dsst.nscrem) future[FSCMOVE] = 0.2777;
		isatb = 0;
		...
		if (damage[DRADIO]==0.0 || stdamtim > dsst.date) {
			for (l = 1; l <= 8; l++)
				for (ll = 1; ll <= 8; ll++)
					if (starch[l][ll] > 1)
						starch[l][ll]=damage[DRADIO]>0.0 ? dsst.galaxy[l][ll]+1000 :1;
...
void probe(void) {
	...
	probecx = quadx;
	probecy = quady;
	future[FDSPROB] = dsst.date + 0.01; // Time to move one sector
...
void helpsst(void) {
	...
	if (basex!=0) {
		...
		ddist = 1e30;
		for (l = 1; l <= dsst.rembase; l++) {
			xdist=10.0*sqrt(square(dsst.baseqx[l]-quadx)+square(dsst.baseqy[l]-quady));
			if (xdist < ddist) {
				ddist = xdist;
				line = l;
			}
		...
		quadx = dsst.baseqx[line];
		quady = dsst.baseqy[line];
		newqad(1);
	}
	...
	for (l = 1; l <= 5; l++) {
		ix = basex+3.0*Rand()-1;
		iy = basey+3.0*Rand()-1;
		if (ix>=1 && ix<=10 && iy>=1 && iy<=10 && quadsst[ix][iy]==IHDOT) {
			/* found one -- finish up */
			prout("succeeds.");
			sectx=ix;
			secty=iy;
